#!/usr/bin/env bash
#
# build_nginx.sh - Build the Cohesity One / T-Pot nginx container and push it to GHCR.
#
# Produces:  ghcr.io/jmousqueton/nginx:24.04.1  (multi-arch: linux/amd64, linux/arm64)
#
# The nginx image bakes in the landing page (index.html, Cohesity One logo) and
# the reverse-proxy config (tpotweb.conf), so rebuild this after changing anything
# under docker/nginx/dist/.
#
# Usage:
#   ./build_nginx.sh                 # build + push (multi-arch)
#   ./build_nginx.sh -n              # build + push with --no-cache
#   ./build_nginx.sh -l              # local single-arch build, load into docker, NO push
#   ./build_nginx.sh -h              # help
#
...
# --- Config -----------------------------------------------------------------
IMAGE="ghcr.io/jmousqueton/nginx"
VERSION="24.04.1"
...
if $LOCAL; then
    ...
    echo "Test it:  docker run --rm -p 64297:64297 ${TAG}"
    exit 0
fi

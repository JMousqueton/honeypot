#!/usr/bin/env bash
#
# build-map.sh - Build the T-Pot / C-Pot Attack Map container and push it to GHCR.
#
# Produces:  ghcr.io/jmousqueton/map:24.04.1  (multi-arch: linux/amd64, linux/arm64)
#
# Usage:
#   ./build-map.sh                 # build + push (multi-arch)
#   ./build-map.sh -n              # build + push with --no-cache
#   ./build-map.sh -l              # local single-arch build, load into docker, NO push
#   ./build-map.sh -h              # help
#
...
# --- Config -----------------------------------------------------------------
IMAGE="ghcr.io/jmousqueton/map"
VERSION="24.04.1"
CONTEXT="docker/elk/map"
...
echo "###########################################"
echo "# Building Attack Map image"
echo "#   image:     ${TAG}"
...
if $LOCAL; then
    ...
    echo "Test it:  docker run --rm -e MAP_COMMAND=AttackMapServer.py -p 64299:64299 ${TAG}"
    exit 0
fi
